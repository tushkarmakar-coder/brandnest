...
export default function Industries() {
  return (
    <section className="bg-[#FAFAF8] py-[110px] px-6 md:py-[120px]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="font-display text-[clamp(32px,4vw,52px)] font-extrabold text-[#111111] leading-[1.1] tracking-tight">
            Tailored Solutions for Every <em className="italic font-normal">Sector</em>
          </h2>
        </motion.div>
          ...
          {VERTICALS.map((v, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white p-8 rounded-xl border border-[rgba(0,0,0,0.05)] shadow-sm hover:shadow-md transition-all duration-300 group"
              >
                <div className="w-12 h-12 rounded-lg bg-[rgba(255,92,0,0.05)] flex items-center justify-center mb-6 group-hover:bg-[#FF5C00] transition-colors duration-300">
                  <Icon size={24} className="text-[#FF5C00] group-hover:text-white transition-colors duration-300" strokeWidth={1.5} />
                </div>
                <h3 className="font-display text-[18px] font-bold text-[#111111] mb-3">{v.title}</h3>
                <p className="text-[13px] text-[rgba(0,0,0,0.5)] leading-relaxed">{v.description}</p>
              </motion.div>
            )
          })}
        </div>
